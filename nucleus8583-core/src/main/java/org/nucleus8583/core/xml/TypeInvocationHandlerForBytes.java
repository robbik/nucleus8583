...
public class TypeInvocationHandlerForBytes implements InvocationHandler {
	...
	public TypeInvocationHandlerForBytes(Type<?> proxied, MessageSerializer serializer) {
	...
	public Object invoke(Object proxy, Method method, Object[] args) throws Throwable {
		if (method.equals(readMethod)) {
			byte[] data = (byte[]) method.invoke(proxied, args[0]);
			
			Message out = new Message();
			serializer.read(data, out);
			
			return out;
		} else if (method.equals(writeMethod)) {
			byte[] bytes = serializer.write((Message) args[1]);
			
			return method.invoke(proxied, args[0], bytes);
		} else {
